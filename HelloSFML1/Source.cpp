...
int main()
{
	...
	// Load 1 new texture
	textureCloud.loadFromFile("graphics/cloud1.png");

	// Make the clouds with arrays
	const int NUM_CLOUDS = 11;
	...
	//prepare frontpage
	Texture texturefrontpage;
	texturefrontpage.loadFromFile("graphics/frontpage.png");
	Sprite spritefrontpage;
	spritefrontpage.setTexture(texturefrontpage);
	spritefrontpage.setPosition(0, 0);
	...
	texturePlayer.loadFromFile("graphics/angry_man.png");
	Sprite spritePlayer;
	...
	spriteAxe.setPosition(600,1080/2);
	...
	// Load 1 new texture
	textureFront.loadFromFile("graphics/4.png");
	// Create a sprite
	Sprite spriteFront;
	// Attach the texture to the sprite
	spriteFront.setTexture(textureFront);
	// Set the spriteBackground to cover the screen
	spriteFront.setPosition(0,0);

	int runtime = 0;
	while (runtime !=100)
	{
		window.clear();
		// Draw our game first scene here
		window.draw(spriteFront);
		window.display();
		runtime++;
	}
	while (window.isOpen())
